...
export function formatEventDateRange(start: Date, end: Date) {
  ...
  return {
    dayRange: isSameDay ? twoDigits(start.getDate()) : `${twoDigits(start.getDate())}-${twoDigits(end.getDate())}`,
    monthRange: isSameMonth ? shortMonths[start.getMonth()] : `${shortMonths[start.getMonth()]}-${shortMonths[end.getMonth()]}`
  };
};